...
def on_select_img(gallery_list_now, event_data: gr.SelectData):
    ...
    return delete_wide.update(visible=True), width_data['max_width'], width_data['avg_width'], width_data[
        'other_width'], finish_data[0:5]
...
def delete_img(img_name_delete, file_path):
    ...
    os.remove(os.path.join(file_path, 'inCircle', img_name_delete.split('.')[0] + '.json'))
    ...
    with gr.Blocks() as demo2:
        with gr.Row():
            with gr.Column():
                ...
                with gr.Column():
                    wide_result_img = gr.Gallery(label="宽度计算结果", value=in_circle_list, columns=5,
                                                 object_fit='contain')
            ...
            with gr.Column():
                ...
                other_width = gr.Dataframe(headers=["次要裂缝宽度"], datatype=["number"], max_rows=5,
                                           overflow_row_behaviour="paginate")
                random_width = gr.Dataframe(headers=["随机取样宽度"], datatype=["number"])
                data_previous = gr.Button("上一页")
                data_next = gr.Button("下一页")

        first_page_wide.click(fn=lambda: update_img_list("首页"),
                              outputs=[wide_result_img, getPageNum_wide, delete_wide])
        beforeButton_wide.click(fn=lambda: update_img_list("上一页"),
                                outputs=[wide_result_img, getPageNum_wide, delete_wide])
        nextButton_wide.click(fn=lambda: update_img_list("下一页"),
                              outputs=[wide_result_img, getPageNum_wide, delete_wide])
        end_page_wide.click(fn=lambda: update_img_list("尾页"),
                            outputs=[wide_result_img, getPageNum_wide, delete_wide])
        refresh_wide.click(fn=lambda: update_img_list("刷新"),
                           outputs=[wide_result_img, getPageNum_wide, delete_wide])
        getPageNum_wide.submit(fn=on_page_num_change, inputs=[getPageNum_wide],
                               outputs=[wide_result_img, getPageNum_wide, delete_wide])

        wide_result_img.select(fn=on_select_img, inputs=[wide_result_img],
                               outputs=[delete_wide, max_width, avg_width, other_width, random_width])
        delete_wide.click(fn=lambda: delete_img(img_name, result_path), outputs=[wide_result_img])
